...
function parseClientOrigins(value) {
  ...
  for (const origin of origins) {
    let url;
    try { url = new URL(origin); } catch { throw new Error(`Invalid frontend origin: ${origin}`); }
    if (!['http:', 'https:'].includes(url.protocol) || url.origin !== origin) {
      throw new Error(`Frontend origins must contain only protocol and host: ${origin}`);
    }
  }
  return origins;
}

const configuredClientOrigins = process.env.CLIENT_ORIGINS || process.env.CLIENT_ORIGIN || process.env.FRONTEND_URL || 'http://localhost:3001';
